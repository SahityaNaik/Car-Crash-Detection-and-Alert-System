...
def detect_from_video():
    ...
    # load the COCO class labels our YOLO model was trained on
        ...
        try:
            ...
            if total <= 0:
                raise Exception("no frames reported")
        ...
        while True:
                ...
                if len(idxs) > 0:
                        ...
                        if(counter==8):
                                ...
                                engine.runAndWait()
                                print("CRASH ALERT")
                                # ensure output directory exists and use consistent lowercase filename
...
# Load background image (try project root first, then `Images/`, fall back to plain background)
background_image = None
background_path = "crash1.png"
if not os.path.exists(background_path):
        alt_path = os.path.join("Images", "crash1.png")
...
if os.path.exists(background_path):
        try:
                ...
                print(f"Couldn't load background image '{background_path}': {e}")
                background_label = tk.Label(root, bg="lightgray")
                background_label.place(relwidth=1, relheight=1)
else:
        # Fallback: simple colored background if no image is found
        print(f"Background image not found (tried '{background_path}'). Using solid background.")
        background_label = tk.Label(root, bg="lightgray")
        background_label.place(relwidth=1, relheight=1)
